...
enum {
	...
	K = 20298703, /* re-tuned for the v0.95 vector op keywords (M=23,
	               * 512-slot perfect hash over all 130 keywords) */
	M = 23,
};
...
parseline(PState ps)
{
	...
	default:
		...
		case Tcall:
		case Ovastart:
		case Ovstore:   /* simple(v0.95): result-less, but not in isstore */
