...
static struct {
	...
	{ Oloaduw, Ki, "ldr %W=, %M0" },
	{ Oload,   Ka, "ldr %=, %M0" },
	/* simple(v0.95): 128-bit vector load/store (q regs) + 2-lane f64
	 * arithmetic (v.2d) + scalar->vector broadcast */
	{ Ovload,  Ka, "ldr %V=, %M0" },
	{ Ovstore, Kw, "str %V0, %M1" },
	{ Ovfadd,  Kd, "fadd %A=.2d, %A0.2d, %A1.2d" },
	{ Ovfsub,  Kd, "fsub %A=.2d, %A0.2d, %A1.2d" },
	{ Ovfmul,  Kd, "fmul %A=.2d, %A0.2d, %A1.2d" },
	{ Ovfdiv,  Kd, "fdiv %A=.2d, %A0.2d, %A1.2d" },
	{ Ovdup,   Kd, "dup %A=.2d, %A0.d[0]" },
	{ Oextsb,  Ki, "sxtb %=, %W0" },
	{ Oextub,  Ki, "uxtb %W=, %W0" },
	{ Oextsh,  Ki, "sxth %=, %W0" },
	{ Oextuh,  Ki, "uxth %W=, %W0" },
...
/* simple(v0.55): conditional select */
#define X(c, str, _) \
	{ Oxsel+c, Ki, "csel %=, %0, %1, " str }, \
	{ Oxsel+c, Ka, "fcsel %=, %0, %1, " str },
	CMP(X)
#undef X
	{ NOp, 0, 0 }
};
...
emitf(char *s, Ins *i, E *e)
{
	...
	Switch:
		...
		case 'V':
		case 'A':
			/* simple(v0.95): a vector register, printed qN (for load/
			 * store, %V) or vN (for .2d arithmetic, %A). Same physical
			 * register V0..V31 an FP value uses; only the view differs. */
			{
				const char *pfx = c == 'V' ? "q" : "v";
				c = *s++;
				assert(c == '0' || c == '1' || c == '=');
				r = c == '=' ? i->to : i->arg[c - '0'];
				assert(isreg(r));
				if (r.val == V31)
					...
					assert(V0 <= r.val && r.val <= V30);
					fprintf(e->f, "%s%d", pfx, (int)(r.val - V0));
				}
			}
			break;
		case 'M':
...
arm64_emitfn(Fn *fn, FILE *out)
{
	...
	for (lbl=0, b=e->fn->start; b; b=b->link) {
		if (lbl || b->npred > 1)
			fprintf(e->f, "%s%d:\n", T.asloc, id0+b->id);
		/* simple(v0.6): a block ending in `cmp Rn, #0` followed by an
		 * eq/ne branch is exactly ARM64's cbz/cbnz — one instruction
		 * instead of two. Detected here, where both halves are visible. */
		nins = b->nins;
		cbz = 0;
		c = b->jmp.type < Jjf ? -1 : b->jmp.type - Jjf;
		if (nins > 0 && (c == Cieq || c == Cine)) {
			i = &b->ins[nins - 1];
			if (i->op == Oacmp && rtype(i->arg[0]) == RTmp
			&& rtype(i->arg[1]) == RCon) {
				pc = &e->fn->con[i->arg[1].val];
				if (pc->type == CBits && pc->bits.i == 0) {
					cbz = 1;
					cbzr = i->arg[0].val;
					cbzk = i->cls;
					nins--;
		...
		default:
			...
			if (b->link == b->s2) {
				...
				n = 1;
			if (cbz)
				/* ctoa[c][1] is the *negated* condition, so the
				 * taken edge tests "eq" when c is Cieq and n is
				 * 0, or when c is Cine and n is 1 */
				fprintf(e->f,
					"\t%s\t%s, %s%d\n",
					(c == Cieq) == (n == 0) ? "cbz" : "cbnz",
					rname(cbzr, cbzk), T.asloc, id0+b->s2->id
				);
			else
				fprintf(e->f,
					"\tb%s\t%s%d\n",
