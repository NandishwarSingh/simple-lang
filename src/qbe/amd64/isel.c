...
sel(Ins i, Num *tn, Fn *fn)
{
	...
	case Ostoreb:
		...
		goto Emit;
	case Ovstore:  /* simple(v0.95): 128-bit vector store */
		seladdr(&i.arg[1], tn, fn);
		goto Emit;
	case Ovload:   /* simple(v0.95): 128-bit vector load */
	case_Oload:
	...
	case Otruncd:
	case Ocast:
	case Ovfadd:  /* simple(v0.95): 2-lane f64 SSE arithmetic + broadcast */
	case Ovfsub:
	case Ovfmul:
	case Ovfdiv:
	case Ovdup:
