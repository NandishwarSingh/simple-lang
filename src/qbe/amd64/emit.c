...
static struct {
	...
	/* simple(v0.95): 128-bit vector load/store + 2-lane f64 arithmetic +
	 * scalar->2-lane broadcast. AVX 3-operand form (no 2-address copy: an
	 * SSE `+mulpd` inserts a movsd that would drop the high lane when a
	 * value is reused). Same xmm registers as the scalar fp ops. */
	{ Ovload,   Ka, "vmovupd %M0, %=" },
	{ Ovstore,  Ka, "vmovupd %D0, %M1" },
	{ Ovfadd,   Kd, "vaddpd %1, %0, %=" },
	{ Ovfsub,   Kd, "vsubpd %1, %0, %=" },
	{ Ovfmul,   Kd, "vmulpd %1, %0, %=" },
	{ Ovfdiv,   Kd, "vdivpd %1, %0, %=" },
	{ Ovdup,    Kd, "vmovddup %0, %=" },
	{ Oload,    Ka, "mov%k %M0, %=" },
