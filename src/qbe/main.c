...
/* simple(v0.55): QBE is embedded in simplec rather than shelled out to.
 * This replaces main(): compile one .ssa file to one .s file, using the
 * host's default target. Errors still exit(1) via QBE's die() path, which
 * is the behavior we want — a backend failure is a compiler failure. */
int
qbe_compile(const char *inpath, const char *outpath, const char *target)
{
	FILE *inf;
	Target **t;
	...
	parse(inf, (char *)inpath, dbgfile, data, func);
	fclose(inf);
	T.emitfin(outf);
	if (fclose(outf) != 0) {
...
int
qbe_unused_main(int ac, char *av[])
{
	Target **t;
	FILE *inf, *hf;
	char *f, *sep;
