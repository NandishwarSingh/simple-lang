...
enum {
	/* simple(v0.55): raised from 2/2. If-conversion trades a possible
	 * ~15-cycle branch misprediction for unconditionally executing both
	 * arms, so a handful of extra ALU ops still wins on unpredictable
	 * branches. Values chosen by the perf lab. */
	MaxIns = 8,
	MaxPhis = 3,
};
